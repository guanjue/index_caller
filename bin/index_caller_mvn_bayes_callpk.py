...
def index_caller(input_signal_mat, output_signal_mat, index_count_lim, std_upper_lim, std_lower_lim, iteration_num_all, alpha_for_empty_index_set, random_seed):
	...
	###### get new labels
	### get old lable matrix
	old_label_mat = []
	for i in range(0, data.shape[0]):
		old_label_mat.append(data[i,1].split('_'))
	label_mat = np.array(old_label_mat)
	label_vec = data[:,1]
	label_vec_uniq = np.unique(label_vec, return_index = False)

	new_label_mat = []
	for i in range(0, ct_num):
		print('cell type: ' + str(i))
		label_vector = old_label_mat[:,i]
		signal_vector = (data[:,2+i]).astype(float)
		signal_null = signal_vector[label_vector=='0']
		new_label = []
		### 
		for index_set in label_vec_uniq:
			index_set_sig_vec = signal_vector[label_vec==index_set]
			### get t-sample test 
			t_stat, tow_side_pval = stats.ttest_ind(a,b, equal_var=False)
			one_side_pval = stats.t.cdf(t_stat, len(a) + len(b) - 2) * 2
			if one_side_pval < 0.05:
				index_ct = '1'
			else:
				index_ct = '0'
			label_vector[label_vec==index_set] = index_ct
