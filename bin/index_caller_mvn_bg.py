...
data_new = read2d_array('atac_20cell.sig.test.txt', str)
...
data_new_sig = data_new[:,1:].astype(float)
data_New_sig_matrix_bed = data_new[:,0]


data = read2d_array('snapshot20_reproduce/atac_20cell.sig.txt', str)
...
for index in data_index_vec:
	...
	if std_max_i > std_lim:
		...
		mvn_qda = -1/2*np.log(np.linalg.det(data_sig_cov_i)) - np.sum(np.dot((data_new_sig-data_sig_mean_i), np.linalg.inv(data_sig_cov_i)) * (data_new_sig-data_sig_mean_i), axis=1) + np.log(data_sig_p_i) #+ constant
	else:
		#print(data_sig_cov_i)
		mvn_qda = -1/2*np.log(np.linalg.det(data_sig_cov_i)) - np.sum(np.dot((data_new_sig-data_sig_mean_i), np.linalg.inv(data_sig_cov_i)) * (data_new_sig-data_sig_mean_i), axis=1) + np.log(data_sig_p_i) #+ constant
...
count_table = open('count_table.txt','w')
for index in data_index_vec:
	print(index)
